...
export function ResourceLoadChart({ resources }: ResourceLoadChartProps) {
    ...
    const aggregation = useMemo((): DailyAggregation => {
        if (!resources.length) return { days: [], resources: [], matrix: [], totalsByRes: [] };
        ...
        if (!minDt || !maxDt) return { days: [], resources: [], matrix: [], totalsByRes: [] };

        const fromDay = parseLocalDateInput(fromDate) || minDt;
        const toDay = parseLocalDateInput(toDate) || maxDt;

        if (fromDay > toDay) return { days: [], resources: [], matrix: [], totalsByRes: [] };
        ...
        return { days, resources: resourceNames, matrix, totalsByRes };
    ...
    const handleBarMouseEnter = (e: React.MouseEvent<SVGRectElement>, tooltipData: { date: string; resource?: string; hours: number; total?: number }) => {
        const content = (
            <div className="bg-gray-900 text-white text-xs p-2 rounded-md shadow-lg">
                <table className="w-full">
                    <tbody>
                        <tr>
                            <td className="text-gray-400 pr-2">Date</td>
                            <td>{tooltipData.date}</td>
                        </tr>
                        ...
                        {tooltipData.total !== undefined && (
                            <tr>
                                <td className="text-gray-400 pr-2">Day Total</td>
    ...
    const handleExportCSV = () => {
        if (!aggregation.days.length) return;
        ...
        const header = ['Date', ...resourceNames];
        const lines = [header.join(',')];
        for (let d = 0; d < aggregation.days.length; d++) {
            lines.push([aggregation.days[d], ...matrix[d].map((v) => v.toFixed(2))].join(','));
        }
    ...
    const renderChart = () => {
        ...
        if (viewMode === 'single') {
            ...
            const barW = 20;
            const step = 26;
            const w = pad.left + pad.right + Math.max(aggregation.days.length * step, 300);
            const h = 340;
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="text-sm text-muted-foreground">
                            Days: {aggregation.days.length} | Max daily hours: {maxDay.toFixed(1)}
                        </div>
                    </div>
                    ...
                    <div className="border rounded-lg bg-white dark:bg-gray-950 overflow-x-auto p-4">
                        <svg width={w} height={h} xmlns="http://www.w3.org/2000/svg">
                            ...
                            {aggregation.days.map((day, d) => {
                                ...
                                return (
                                    <rect
                                        ...
                                        onMouseEnter={(e) => handleBarMouseEnter(e, { date: day, resource: resName, hours: val })}
                            ...
                            {/* X-axis labels */}
                            {aggregation.days.map((day, d) => {
                                if (d % Math.max(1, Math.ceil(aggregation.days.length / 12)) !== 0) return null;
            ...
            return (
                <div className="space-y-4">
                    ...
                    <div className="text-sm text-muted-foreground">
                        Days: {aggregation.days.length} | Resources: {resourceNames.length}/{aggregation.resources.length} | Max
                        daily hours: {maxDay.toFixed(1)}
                    </div>
                                        ...
                                        {resourceNames.map((resName, r) => {
                                            ...
                                            const rect = (
                                                <rect
                                                    ...
                                                    className="hover:opacity-80 cursor-pointer transition-opacity"
                                                    onMouseEnter={(e) => handleBarMouseEnter(e, { date: day, resource: resName, hours: val, total: totalsByDay[d] })}
                                                    onMouseMove={handleBarMouseMove}
                                                    onMouseLeave={handleBarMouseLeave}
                                                />
                                            );
