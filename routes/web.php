...
Route::middleware(['auth', 'verified'])->group(function () {
    ...
    Route::get('projects/{project}/schedule-variants/{scheduleVariant}/task_schedule.csv', function (
        Project $project,
        ScheduleVariant $scheduleVariant,
    ) {
        abort_if($scheduleVariant->project_id !== $project->id, 404);

        if (! $scheduleVariant->task_path) {
            abort(404);
        }
    ...
    Route::get('projects/{project}/schedule-variants/{scheduleVariant}/resource_tracking.csv', function (
        Project $project,
        ScheduleVariant $scheduleVariant,
    ) {
        abort_if($scheduleVariant->project_id !== $project->id, 404);

        if (! $scheduleVariant->resource_path) {
            abort(404);
        }
...
Route::get('schedule-viewer/variant/{scheduleVariant:slug}/task_schedule.csv', function (ScheduleVariant $scheduleVariant) {
    foreach ($scheduleVariant->taskStorageCandidates() as $candidate) {
...
Route::get('schedule-viewer/variant/{scheduleVariant:slug}/resource_tracking.csv', function (ScheduleVariant $scheduleVariant) {
    foreach ($scheduleVariant->resourceStorageCandidates() as $candidate) {
        $path = 'private/'.$candidate;
